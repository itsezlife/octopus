...
void main() => group('state', () {
  ...
  test('encode_and_decode_cyrillic_location', () {
    ...
    final encodedLocation = StateUtil.encodeLocation(sourceState).toString();
    ...
    expect(
      ...
      equals({'text': 'Привет мир'}),
    );
  });

  test('decode_and_encode_cyrillic_state', () {
    const location = 'route~name=Привет мир?теплое=Мягкое&Вкусное=кислое&флаг';
    final state = StateUtil.decodeLocation(location);
    expect(state.children, hasLength(1));
    expect(
      state.children.single,
      isA<OctopusNode>()
          .having((e) => e.name, 'name', allOf(isNotEmpty, equals('route')))
          .having(
            (e) => e.arguments,
            'arguments',
            allOf(isNotEmpty, equals({'name': 'Привет мир'})),
          ),
    );
  ...
  test('decode_location_with_uuid', () {
    ...
    expect(
      ...
      isA<OctopusNode>()
          .having((e) => e.arguments['id'], 'id', allOf(isNotEmpty, equals(id)))
          .having(
            (e) => e.arguments['selected'],
            'selected',
            allOf(isNotEmpty, equals(selected)),
          ),
    );
  ...
  test('decode_url_to_state', () {
    ...
    expect(state.children.length, equals(2));
  });
});
